...
**Revised:** 2026-09-25. The draft step is removed (uploads go straight to review). ETL takes a
row and column selection. Flagged rows of a rejected file go to the Incomplete data page. Staff
get notified when a supplier submits.
...
| Upload-time blocking | Removed; every upload goes straight to the vendor's queue as `PENDING`, and the vendor runs ETL (there is no draft step) |
...
`REJECTED` (whole file rejected) is set explicitly, never derived. An upload with zero rows cannot exist (the upload is rejected at
parse time).

### Upload time

`createUpload` creates the upload as `PENDING`, so it enters the vendor's queue at once, and
notifies the organization's upload reviewers (see section 5). Parse issues and missing values
are still stored in `issues`/`missingFields` so the supplier sees the flags on the upload.
There is no draft status or separate submit step. `initialUploadStatus` is removed. Rejection resubmits
...
- `findEmptyColumns(row, columns?)` returns the names of columns whose value is empty. With
  `columns`, only those columns are checked, plus `sku` and `order_qty`, which are always
  checked because stocking needs them (`REQUIRED_COLUMNS`). Text fields
...
- `classifyRows(rows, existingSkus, columns?)` returns, for each row, `{ status: GOOD | INCOMPLETE, missing: string[] }`,
...
|---|---|---|
| `POST .../etl` | `{ itemIds?, columns? }` | Classifies the selected NEW rows (all NEW rows if `itemIds` is omitted) into GOOD/INCOMPLETE, checking only the selected CSV columns (all 12 if `columns` is omitted; `sku` and `order_qty` are always checked). Writes `missingFields`, then re-derives the upload status. Unselected rows stay NEW. Returns `{ good, incomplete }` counts for this run. Fails if the upload is not PENDING or a listed id is not NEW. |
| `POST .../accept` (changed) | `{ locationId, itemIds?, fields? }` | Stocks **GOOD** rows only (all GOOD rows if `itemIds` is omitted; the call fails if any listed id is not GOOD). Rows become STOCKED with `stockedLocationId`/`stockedAt`. Product upsert, column selection, and inventory movement work as today. An empty or zero value never overwrites an existing product's name, category, or cost price, since a column skipped at ETL can be empty on a GOOD row. Sets `upload.locationId` to this location and `acceptedAt`/`acceptedById` on first stocking. Re-derives the status and triggers the AI pipeline. |
...
- **New tab:** row checkboxes and a checkbox on every column header choose what ETL checks
  (SKU and Qty are always checked and their boxes are disabled). A "Run ETL on N rows" button
  runs it. After a run, a result strip reads "82 good · 18 incomplete". The column choice
  carries over to the Good tab.
...
- **Good tab:** row and column selection (unchecked description, category, or unit price are not
  written to the catalog), a store picker, and an "Add N rows to stock" button. Can be repeated
...
| Method & path | Body | Effect |
|---|---|---|
| `GET /supplier-portal/incomplete-items` | none | Rows waiting for this supplier's correction, with `upload { id, originalName, vendorNote, status }`, ordered by upload creation time and then row id. That means RETURNED rows of PENDING/INCOMPLETE uploads, plus RETURNED or INCOMPLETE rows of a REJECTED upload (rows ETL flagged before the vendor rejected the whole file). |
| `POST /supplier-portal/incomplete-items/resubmit` | `{ items: [{ id, poNumber, vendorId, vendorName, sku, itemDescription, category, orderQty, unitPriceBdt, totalAmountBdt, orderDate, deliveryDate, status }] }` (min 1) | Every id must be a row waiting for this supplier's correction (as listed above). Runs `findEmptyColumns` on each edited row; if any row fails, nothing is saved and a 400 lists `{ id, fields[] }` per failing row. Otherwise the edits are saved, rows become NEW with `missingFields = []`, each affected upload's status is re-derived (so it returns to PENDING), `submissionCount` is incremented once per affected upload, and one `UPLOAD_RECEIVED` notification is sent per affected upload. A REJECTED upload is reopened as PENDING (`rejectedAt` cleared); its flagged rows that were not in this batch become RETURNED so they stay on the supplier's page. |

`GET /supplier-portal/me` report counts gain `returnedItems` (the number of rows the list above
returns), which drives the sidebar badge.

### UI

- A sidebar item "Incomplete data" (after "My uploads") with a count badge when greater than 0.
- Rows are grouped by upload. Each group header shows the file name and the vendor's note. A
  rejected file is marked "File rejected", with a line explaining that resubmitting its rows
  sends the whole file back for review.
...
Supplier notifications (`SupplierNotification`) are unchanged in meaning. Staff get their own:

- **`VendorNotification`**: one row per recipient, with `type` (`UPLOAD_SUBMITTED`,
  `UPLOAD_RESUBMITTED`, `ROWS_RESUBMITTED`), `message`, and `readAt`. Recipients are the
  organization's active users whose role has `supplier-uploads.manage`.
- **Created** in the same transaction as the supplier action: a new upload, a rejected file
  resubmitted (as a file or with edits), and corrected rows resubmitted from the Incomplete data
  page.
- **Endpoints** (the signed-in staff user's own rows only): `GET /notifications`,
  `POST /notifications/:id/read`, `POST /notifications/read-all`.
- **Live updates:** `GET /supplier-portal/notifications/stream` (supplier) and
  `GET /notifications/stream` (staff) are Server-Sent Events streams, fanned out through Redis
  pub/sub. They publish only after the transaction commits. The browser refetches on each event
  and shows a toast for new notifications.
- **UI:** the staff sidebar has a "Notifications" item with an unread badge, and a
  `/notifications` page links each entry to its upload's review page.
...
- `etl.util.spec.ts`: `findEmptyColumns` (blank text, whitespace, zero/missing numbers, a fully filled row,
  column selection with SKU/Qty always checked); `classifyRows` (good vs. incomplete, duplicate SKUs within
  the batch and against existing GOOD/STOCKED rows, case-insensitive).
- `accept-selection.util.spec.ts`: row selection by status, and the ETL request schema.
- `vendor-notifications.spec.ts`: which roles are notified.
